 ...
 */
static void init(const int n, const int m, double* const grid) {
  ...
  for (int i = 0; i < n; i++) {
    double col0 =
        0.98 * (double) ((i + 1) * (i + 1)) /
        (double) (n * n); // Set it as a variable to make initialisation easier
    grid[i * increment + 0] = col0; // We have a '+ 0' here for consistency

    // Interior points
    for (int j = 1; j < m; j++) {
      grid[i * increment + j] =
          col0 * ((double) (m - j) * (m - j) / (double) (m * m));
    }
 ...
 */
static void iteration(const int m, double* const restrict dst,
  ...
  // Here, we deal with the column at j = 1, since it also wraps around
  {
    ...
    // Left neighbours
    double old_l2 = src[m - 1];
    double old_l1 = src[0];

    // Right neighbours
    double old_r1 = src[j + 1];
    double old_r2 = src[j + 2];
 ...
 */
static void iterations(const int iters, const int m, double* restrict dst,
                       double* restrict src) {
  if (iters % 2 != 0) {
    iteration(m, dst, src);
  }
...
void heat_propagation(const int iters, const int n, const int m,
  ...
  for (int i = 0; i < n; i++) {
    double* const row_dst = dst + i * increment;
    double* const row_src = src + i * increment;
